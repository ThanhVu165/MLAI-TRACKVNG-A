...
## 4. Git & Quy trình Đồng bộ Nhánh

- Nhánh tích hợp chính đang phát triển: `dev`. Nhánh phát hành chính thức: `main`.
- Nhánh làm việc của từng thành viên/agent: `agent-a/<task-id>-<slug>`, `agent-b/...`, `agent-c/...` rẽ từ `dev`.
- **CẤM `git rebase -i` để squash. CẤM `git push --force`.** Brief ghi rõ: *hành động gộp commit hoặc ghi đè lịch sử sẽ bị tính là không hợp lệ — lịch sử commit là bằng chứng đánh giá quá trình phát triển.*
- Khi hoàn thành task: Rebase/pull nhánh `dev` mới nhất về nhánh của mình, chạy toàn bộ test suite và linting, sau đó merge commit hoặc push vào `dev`.
- Commit thường xuyên, có ý nghĩa, format:
...
- Không commit: `data/app.db`, `data/*.db`, `.env`, `__pycache__`, `.venv`, file `embedding_cache/`, khóa API.
...
## 10. Kiểm thử & Cổng Chất lượng (Quality Gate)

- `pytest tests/`: Toàn bộ 169 unit/integration tests trên cả 3 làn A, B, C phải PASS 100%.
- Bắt buộc có test cho: Policy Engine (mọi luật P01–P05), Ground Guard (4 kiểm tra), Question Guard (blocklist), Chunker (giữ được Điều/Khoản), Conflict detection, Harness (gọi đúng `process_case`).
- Test LLM dùng **cassette**: `infra/llm.py` có chế độ `LLM_MODE=replay` đọc phản hồi đã ghi trong `tests/cassettes/`. Môi trường test và offline hoàn toàn không gọi mạng.
- Fallback tinh gọn (Ponytail): `corpus/indexer.py` tích hợp sẵn `DefaultEmbedder` chạy trên numpy/stdlib để test và chạy thử nghiệm mà không cần cài đặt gói nặng 500MB `sentence-transformers/torch`.
...
- **Cổng nghiệm thu commit bắt buộc:**
  ```powershell
  python -m ruff check .
